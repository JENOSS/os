...
static long randNum = 1L;

static int ScheduleArray[20] = {0,};
static int GraphCheck = 0;
static int order = 0;
static int timecheck = 100;

static int fairnessGraph[3][10] ={ 0,  };
static int fairness_GraphCheck =0;
static int fairness_order =0;
static int fairness_timecheck =100;
 ...
 */
static TCB* kGetNextTaskToRun_1( void )
{
    TCB* pstTarget = NULL;
    int iTaskCount, i, j;
...
// Lottery 
static TCB* kGetNextTaskToRun_2( void )
{
    TCB* pstTarget = NULL;
    int i, j, k;
	...
	int ticket[ TASK_MAXREADYLISTCOUNT ] = {80, 60, 40, 20, 10 };
	int TaskCount[ TASK_MAXREADYLISTCOUNT ] = {0, };
	...
	for( i = 0; i < TASK_MAXREADYLISTCOUNT ; i++ ){
            TaskCount[ i ] = kGetListCount( &( gs_stScheduler.vstReadyList[ i ] ) );
	    TotalTicketCount += ticket[i] * TaskCount[ i ];
	}
	if(TaskCount[1] == 0 && TaskCount[2] == 0 && TaskCount[3] == 0 ) {
	
		return kGetNextTaskToRun_1();
	}
	
	//Rand func
	SelectedTicket = rand() % TotalTicketCount;

	for( i = 0; i < TASK_MAXREADYLISTCOUNT; i++ ){

		for( k = 0; k < TaskCount[i] ; k++ ) { 
		
			SelectedTicket -= ticket[i];
			if( SelectedTicket < 0 ) {
				pstTarget = ( TCB* ) kRemoveList_2( 
					&( gs_stScheduler.vstReadyList[ i ] ) , k );
...
// Stride
static TCB* kGetNextTaskToRun_3( void )
{
    TCB* pstTarget = NULL;
    int i, j, k;
    
	int stride[TASK_MAXREADYLISTCOUNT] = {6, 8, 12, 24 ,48};
	int TaskCount[ TASK_MAXREADYLISTCOUNT ] = {0, };
	int minPass = -1;
	int minX,minY;
	TCB* minPassTCB;
	LISTLINK* minPassLinkList;
	for( i = 0; i < TASK_MAXREADYLISTCOUNT ; i++ ){
        TaskCount[ i ] = kGetListCount( &( gs_stScheduler.vstReadyList[ i ] ) );
	}
	...
	for(i=0 ;i < TASK_MAXREADYLISTCOUNT; i++)
	{
		for(j =0 ; j < TaskCount[ i ] ; j++)
		{
			...
			if(minPass == -1)
			{
				minPassTCB  =(TCB*) kGetNext(& (gs_stScheduler.vstReadyList[i]),j);
				minPass = minPassTCB->pass ;
				minX = i;
				minY = j;
			...
			{
			 	temp  = (TCB*) kGetNext(& (gs_stScheduler.vstReadyList[i]),j);
				nowPass = temp->pass;
				if(nowPass < minPass)
				{
					minPass = nowPass;
	...
	minPassTCB->pass += stride[minX];
	pstTarget = (TCB*)kRemoveList_2( &( gs_stScheduler.vstReadyList[ minX ] ) , minY ) ;


/*		for(i = 0; i< 5; i++)
		kPrintf("%d ",TaskCount[i]);
	kPrintf("\n");
*/
	//kPrintf("%d\n",minPass);
	return pstTarget;
...
void kSchedule( void )
{
   ...
   //pstNextTask = kGetNextTaskToRun_1(); //MLFQ
    pstNextTask = kGetNextTaskToRun_2();  //Lottery
   //pstNextTask = kGetNextTaskToRun_3(); //STride
    if( pstNextTask == NULL )
    {
        // 임계 영역 끝
        kUnlockForSystemData( bPreviousFlag );
...
int rand()
{
	return ( ( ( randNum = randNum*214013L + 2531011L ) >> 16 ) & 0x7fff );


}
void ShowTaskGraph( void ){
	if(timecheck == 0)
	{
		for(int i=0;i<order;i++){
			kPrintf(" p%d ", ScheduleArray[i]);
		}kPrintf("\n");
	}
	else
		kPrintf("testing yet \n");
...
{
	if(GraphCheck == 1 && timecheck > 0)
	{
		ScheduleArray[order] = kTaskID();
		order++;
...
void fairness_ShowTaskGraph( void ){
	if(fairness_order == 10 && fairness_timecheck ==0)
	{
		for(int i=0;i<fairness_order;i++)
		{	
			for(int j=0;j<3;j++)
				kPrintf("p%d : %d ",j+2,fairnessGraph[j][i]);
			kPrintf("\n");
		}
	}
	else 
...
void fairness_GraphCheckZero( void ) {
	fairness_GraphCheck=1;
	fairness_order=0;
}
void fairness_CheckAndSet( void ){
	if(fairness_GraphCheck == 1)
	{
		fairnessGraph[kTaskID()-2][fairness_order]++;
		fairness_timecheck = fairness_timecheck -5;
		if(fairness_timecheck == 0)
		{	
			fairness_order++;
			if(fairness_order < 10){
				fairness_timecheck = 100;
				for(int i=0;i<3;i++)
					fairnessGraph[i][fairness_order]= fairnessGraph[i][fairness_order-1]; 
			}
			else
				fairness_GraphCheck =0;
		}
...
int kTaskID( void ) {
	return gs_stScheduler.pstRunningTask->stLink.qwID;
}
