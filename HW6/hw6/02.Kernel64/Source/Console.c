...
//ScreenSave
BYTE tempScreen[10000] = { 0, };
static BOOL k_ok = TRUE;
static BOOL Screen_ok = FALSE;
...
void kInitializeConsole( int iX, int iY )
{
	...
	//tempScreen = (BYTE*)kAllocateMemory(10000);
	//kMemSet(tempScreen, 0, 10000);
    // 커서 위치 설정
    kSetCursor( iX, iY );
}
    ...
    while( 1 )
    {
        ...
        if( stData.bFlags & KEY_FLAGS_DOWN )
        {
			...
			if (Screen_ok) {
				...
				for (i = 0; i < CONSOLE_WIDTH * CONSOLE_HEIGHT; i++)
				{
					 pstScreen[i].bCharactor = tempScreen[i];
...
void kScreenSaver(void)
{
	...
	if (!Screen_ok ) {
		...
		for (i = 0; i < CONSOLE_WIDTH * CONSOLE_HEIGHT; i++)
		{
			tempScreen[i] = pstScreen[i].bCharactor;
			pstScreen[i].bCharactor = ' ';
			pstScreen[i].bAttribute = CONSOLE_DEFAULTTEXTCOLOR;
		}
