...
static int keyboardtime = 3000;
static int time = 0;
static BOOL k_flag = FALSE;
...
//	kPrintf("%d\n", SelectedTicket);
	for( i = 0; i < TASK_MAXREADYLISTCOUNT; i++ ){
		...
		for( k = 0; k < TaskCount[i] ; k++ ) { 
			...
			if( SelectedTicket <= 0 ) {
				pstTarget = ( TCB* ) kRemoveListForLottery( 
...
void kSchedule( void )
{
    ...
    pstNextTask = kGetNextTaskToRun_1();
   // pstNextTask = kGetNextTaskToRun_2();
   // pstNextTask = kGetNextTaskToRun_3();
    if( pstNextTask == NULL )
    {
        // 임계 영역 끝
        kUnlockForSystemData( bPreviousFlag );
