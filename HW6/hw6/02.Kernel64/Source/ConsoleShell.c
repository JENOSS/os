...
SHELLCOMMANDENTRY gs_vstCommandTable[] =
{
	...
	{ "showtaskgraph", "show task graph", kShowTaskGraph },
	{ "testscheduling", "start scheduling test", kTestScheduling },
	{ "Setkeyboardtime", "keyboardtime", kSetKeyboardtime },
	{ "screensave", "on/off screensave", kSetScreenSave },
};                                     
...
static void kShowTaskGraph( const char* pcParameterBuffer )
{
	//ScheduleArray[order]
	ShowTaskGraph();
}
...
static void kTestScheduling( const char* pcParameterBuffer )
{
    PARAMETERLIST stList;
    char vcType[ 30 ];
    char vcCount[ 30 ];
    ...
    GraphCheckZero(); 
    kPrintf("Start Task Scheduling Test \n");


}
static void kSetKeyboardtime(const char* pcParameterBuffer)
{
	//-1
	char vcParameter[100];
	PARAMETERLIST stList;
	...
	time = kAToI(pcParameterBuffer, 10);
	if (time < 0) {
		kPrintf("ex) 100(ms)\n");
		return;
	}
	kPrintf("%d time set\n", time);
	SetKeyboardtime(time);

}
static void kSetScreenSave(const char* pcParameterBuffer)
{
	char vcParameter[100];
	PARAMETERLIST stList;
	int temp;
	...
	if (kGetNextParameter(&stList, vcParameter) == 0)
	{
		kPrintf("ex) 1(on)/0(off) \n");
		return;
	}

	temp = kAToI(pcParameterBuffer, 10);
	kPrintf("%d\n", temp);
	if (temp == 0) {
		kPrintf("ScreenSave Off\n");
		k_ok_F();
	}
	else if (temp == 1) {
		kPrintf("ScreenSave On\n");
		k_ok_T();
	}
	else {
		kPrintf("ex) 1(on)/0(off) \n");
