...
// for next
void* kGetNext( const LIST* pstList, int num ) 
{
	LISTLINK* Head;
	LISTLINK* want;

	Head = ( LISTLINK* ) kGetHeaderFromList( pstList );
	...
	for( int i = 0; i< num; i++ ) {
		Head = ( LISTLINK*) kGetNextFromList( pstList, Head );
	}
	//want =( LISTLINK*) Head;
	return Head;
}
...
//
void* kRemoveListForLottery( const LIST* pstList, int num) 
{
    LISTLINK* pstLink;
