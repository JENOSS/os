...
SHELLCOMMANDENTRY gs_vstCommandTable[] =
{
		...
		{ "makedir", "make directory ex)makedir child", kMakeDir },
		{ "rmdir", "remove directory ex)rmdir child",kRmDir },
		{ "cd", "Change Directory ex) cd child", kChangeDirectory },
...
void kStartConsoleShell( void )
{
   ...
   // char RootDirBuffer[50];
    RootDirBuffer[0] ='R'; RootDirBuffer[1] ='O'; 
    RootDirBuffer[2] ='O'; RootDirBuffer[3] ='T'; RootDirBuffer[4] ='\0';
	
    // 프롬프트 출력
    if(	 kFindAbsolWrapper( RootDirBuffer, 0) == -2 ) { //루트디렉터리가 있는가
	setFormatNum(0);
    	kCreateDirWrapper( RootDirBuffer );	
	RootDirBuffer[0] ='R'; RootDirBuffer[1] ='O'; 
   	RootDirBuffer[2] ='O'; RootDirBuffer[3] ='T'; RootDirBuffer[4] ='\0';
	kChangeDirectory(RootDirBuffer);
    }
    else {				  // getDirNum() == 0 
	kChangeDirectory( RootDirBuffer); //루트로 이동
    }
    nowDirBuffer[0] = '/';  nowDirBuffer[1] = '\0';
    kPrintf("i'm intial : %s\n", nowDirBuffer);

    kPrintf( CONSOLESHELL_PROMPTMESSAGE );
    kPrintf( "%s> ", nowDirBuffer );    
...
static void kRmDir( const char* pcParameterBuffer)
{
   ...
   /* for(i =0 ; i<stList.iLength ; i++ ){
		vcDirName[i] = pcParameterBuffer[i];
	}*/
    vcDirName[ stList.iLength ] = '\0';
	...
	if( kMemCmp( vcDirName, ".", iLength ) == 0 ){ //현재 위치 경로		
		kPrintf("It's YOURSELF!! Can't do this\n");
	}
	else if( kMemCmp( vcDirName, "..", iLength ) == 0 ){ //부모 위치 경로
		kPrintf("It's YOURPARENT!! Can't do this\n");
	}
	else{
		kRemoveDirWrapper(vcDirName);
	}
}
...
static void kMakeDir( const char* pcParameterBuffer )
{
   ...
   /* for(i =0 ; i<stList.iLength ; i++ ){
	...
	//kPrintf("nowDirBuffer : %s\n", nowDirBuffer);
	//kPrintf("nowDirOffset : %d\n", getDirNum() );

	  	kCreateDirWrapper(vcDirName);
}
...
static void kChangeDirectory( const char* pcParameterBuffer )
{
	...
	//kPrintf("now Index : %d\n", getDirNum() );
        ...
        //현재 위치 경로
	if( kMemCmp( vcDirName, ".", iLength ) == 0 ){ 
				
		if( kMoveDirWrapper( vcDirName ) == -1){
			kPrintf("don't have '.'\n");
		}
	}
	//부모 위치 경로
	else if( kMemCmp( vcDirName, "..", iLength ) == 0 ){ 
		if( kMoveDirWrapper( vcDirName ) != -1) { 
 			
			int mylen = kStrLen(nowDirBuffer)-1;
			if( mylen==0 && nowDirBuffer[0]=='/') { return; }
		
			for( int i=mylen; i>=0; i-- ) {
 				if(nowDirBuffer[i] == '/') {
			  		
					for( int k=i; k<=mylen; k++ ){
						if(k==0){ continue; }	
						nowDirBuffer[k] = '\0'; 
					}
                	      		return;
	...
	//절대 위치 경로
        else if( kMemCmp( vcDirName, "/", 1 ) == 0 ) {
		//kPrintf(" root addr : %s\n",vcDirName);
		if( DisPath( vcDirName ) == -1 ){
			kPrintf(" **Wrong Absolute Path**\n");
		}
	...
	//상대 위치 경로
		...
		if( kMoveDirWrapper( vcDirName ) != -1) {
			...
			int mylen = kStrLen(nowDirBuffer); 
   			int addlen = kStrLen(vcDirName)-1;
			int sumlen =  mylen + addlen;
			if( !(mylen==1 && nowDirBuffer[0]=='/') )		
		    		nowDirBuffer[mylen++] = '/';
			for(int i=0; i<=addlen; i++){
				nowDirBuffer[mylen+i] = vcDirName[i];
				if(i==addlen)
					nowDirBuffer[mylen+i+1] = '\0';
			}

		}
		else if( DisPath_releative( vcDirName ) == 1 ) {
			//kPrintf("success!\n");
		}			
		else{
			kPrintf("not here in this directory\n");
		}
...
static int DisPath_releative( const char* Path )
{
	char tempPath[ 25 ];
	int len = kStrLen(Path);
	int pCount = 0;
	int startIndex=0, finishIndex=0, tempIndex=0, pathCount=0;
	int myDirNum = getDirNum();

	//char tempDirBuffer[50];
	RootDirBuffer[0] ='R'; RootDirBuffer[1] ='O'; 
	RootDirBuffer[2] ='O'; RootDirBuffer[3] ='T';  
	RootDirBuffer[4] ='\0';
	...
	for(int i=0; i<len; i++){
		if(Path[i] == '/'){
			...
			for(int j=startIndex; j<finishIndex; j++){
				tempPath[tempIndex] = Path[j];
				tempIndex++;
			}
			tempPath[tempIndex] = '\0';
			kMemCpy( tryPath[pathCount], tempPath, kStrLen(tempPath)+1 );
			pathCount++; tempIndex=0;
			kMemSet( tempPath, '\0', 25 );
	...
	char nowBuff[300];
	int BuffLen = kStrLen(nowDirBuffer);
	
	for(int i=0; i<BuffLen; i++) {
		nowBuff[i] = nowDirBuffer[i];
	}
	nowBuff[BuffLen] = '\0';

	//kPrintf(" nowDirBuffer : %s\n", nowBuff);
	if(BuffLen == 1 && nowBuff[0] == '/') {
		BuffLen = 0;
		nowBuff[0] = '\0';
	}
	//kPrintf(" nowBuffer : %s\n", nowBuff);	
	...
	for(int i=0; i<pathCount; i++) {
		if( kMemCmp( tryPath[i], ".", kStrLen(tryPath[i]) ) == 0 ){ 
			continue;
		}
		else if( (kMemCmp( tryPath[i], "..", kStrLen(tryPath[i]) ) == 0 ) && BuffLen>0){ 
			//kPrintf("PARENT~\n");			
			for(int k= BuffLen-1; k>=0; k--) {
				if(nowBuff[k] == '/'){
					nowBuff[k] = '\0';
					BuffLen--;
					break;				
		...
		else if ( kMemCmp( tryPath[i], "..", kStrLen(tryPath[i]) ) != 0 ){
			int tLen = kStrLen(tryPath[i]);	
			int bLen = BuffLen;
			//kPrintf(" t:%d d:%d\n",tLen, bLen);
...
//	kPrintf("NICE!!!\n");
	...
	kPrintf("want to show : %s\n", nowBuff);

	kPrintf("pCount : %d\n\n", pCount); */
	
	int isgood = myDirNum ; 
	int tempGood = kFindAbsolWrapper(tryPath[0], myDirNum); //ROOT
	...
	for(int i=1; i<pathCount; i++){
		if(isgood==-2)
			return -1;
		tempGood = kFindAbsolWrapper(tryPath[i], isgood);
		
		if(tempGood == -1) continue;	
		isgood = tempGood;
	}
	if(isgood==-2)	{
		return -1;
		...
		for(int i=0; i<BuffLen; i++)
			nowDirBuffer[i] = nowBuff[i];
		nowDirBuffer[BuffLen] ='\0';
	}
...
static int DisPath( const char* Path )
{
	char tempPath[ 25 ];
	int len = kStrLen(Path);
	int pCount = 0;
	int startIndex=1, finishIndex=1, tempIndex=0, pathCount=0;
	...
	int isgood= kFindAbsolWrapper(RootDirBuffer, 0); //ROOT
	isgood = kFindAbsolWrapper(tryPath[1], isgood);
	
	for(int i=2; i<pathCount; i++){
		if(isgood==-2)
			return -1;
		
		isgood = kFindAbsolWrapper(tryPath[i], isgood);
	}
	if(isgood==-2)	{
		return -1;
	}
...
static void kShowRootDirectory( const char* pcParameterBuffer )
{
    ...
    // 루트 디렉터리를 엶
    pstDirectory = opendir( nowDirBuffer );
    ...
    while( 1 )
    {
        ...
        kMemCpy( vcBuffer + 20, vcTempValue, kStrLen( vcTempValue ) );	//30

        // 파일의 시작 클러스터 삽입
        kSPrintf( vcTempValue, "0x%X Cluster", pstEntry->dwStartClusterIndex );
        kMemCpy( vcBuffer + 33, vcTempValue, kStrLen( vcTempValue ) + 1 );	//55
       // kPrintf( "    %s\n", vcBuffer );
		}
		else {
			kMemCpy(vcBuffer,pstEntry->dd_name,kStrLen( pstEntry->dd_name));
			vcBuffer[kStrLen(pstEntry->dd_name)+60] = '\0';
			kSPrintf( vcTempValue, "DIRECTORY", pstEntry->dwFileSize );
       		kMemCpy( vcBuffer + 20, vcTempValue, kStrLen( vcTempValue ) );	//30
	       // kPrintf( "    %s\n", vcBuffer );
		
		}

	kSPrintf( vcTempValue, "  %d-%d-%d  %dh %dm %ds", pstEntry->wYear, pstEntry->bMonth, pstEntry->bDayOfMonth, pstEntry->bHour, pstEntry->bMinute, pstEntry->bSecond);
