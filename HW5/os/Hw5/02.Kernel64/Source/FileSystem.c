...
//static BYTE nowDirBuffer[ FILESYSTEM_SECTORSPERCLUSTER * 512 ];

//static DWORD nowDirOffset = 0;
...
BOOL kFormat( void )
{
    ...
    // 링크 데이터는 4바이트이므로, 한 섹터에는 SECTORNUM개가 들어감. 따라서 총 개수를
    // SECTORNUM로 나눈 후 올림하여 클러스터 링크의 섹터 수를 구함
    dwClusterLinkSectorCount = ( dwMaxClusterCount + 127 ) / SECTORNUM;
    ...
    dwClusterLinkSectorCount = ( dwClsuterCount + 127 ) / SECTORNUM;
...
static DWORD kFindFreeCluster( void )
{
    ...
    for( i = 0 ; i < gs_stFileSystemManager.dwClusterLinkAreaSize ; i++ )
    {
        ...
        {
            dwLinkCountInSector = gs_stFileSystemManager.dwTotalClusterCount % SECTORNUM; 
        }
        else
        {
            dwLinkCountInSector = SECTORNUM;
        }
        ...
        if( j != dwLinkCountInSector )
        {
            ...
            return ( dwCurrentSectorOffset * SECTORNUM ) + j;
        }
    }
...
static BOOL kSetClusterLinkData( DWORD dwClusterIndex, DWORD dwData )
{
    ...
    // 한 섹터에 SECTORNUM개의 클러스터 링크가 들어가므로 128로 나누면 섹터 오프셋을 
    // 구할 수 있음
    dwSectorOffset = dwClusterIndex / SECTORNUM;
    ...
    ( ( DWORD* ) gs_vbTempBuffer )[ dwClusterIndex % SECTORNUM ] = dwData;
...
static BOOL kGetClusterLinkData( DWORD dwClusterIndex, DWORD* pdwData )
{
    ...
    // 한 섹터에 SECTORNUM개의 클러스터 링크가 들어가므로 SECTORNUM로 나누면 섹터 오프셋을 
    // 구할 수 있음
	dwSectorOffset = dwClusterIndex / SECTORNUM;
    ...
    *pdwData = ( ( DWORD* ) gs_vbTempBuffer )[ dwClusterIndex % SECTORNUM ];
    return TRUE;
}
...
static int kFindFreeDirectoryEntry( void )
{
    ...
    // 루트 디렉터리를 읽음
    if( kReadCluster( nowDirOffset, gs_vbTempBuffer ) == FALSE )
    {
        return -1;
    }  
...
static int kFindFreeDirectoryEntry2( DWORD dwClusterIndex )
{
    DIRECTORYENTRY* pstEntry;
    int i;
...
static BOOL kSetDirectoryEntryData( int iIndex, DIRECTORYENTRY* pstEntry )
{
    ...
    // 루트 디렉터리에 씀
    if( kWriteCluster( nowDirOffset, gs_vbTempBuffer ) == FALSE )
    {
        return FALSE;
    }    
    return TRUE;
...
static BOOL kSetDirectoryEntryData2(DWORD clusterNum, int iIndex, DIRECTORYENTRY* pstEntry )
{
    DIRECTORYENTRY* pstRootEntry;
    ...
    //  디렉터리를 읽음
    if( kReadCluster( clusterNum, gs_vbTempBuffer ) == FALSE )
    {
        return FALSE;
    }    
    ...
    // 디렉터리에 씀
    if( kWriteCluster( clusterNum, gs_vbTempBuffer ) == FALSE )
    {
        return FALSE;
    }    
    return TRUE;
...
static int kFindDirectoryEntry( const char* pcFileName, DIRECTORYENTRY* pstEntry )
{
    ...
    for( i = 0 ; i < FILESYSTEM_MAXDIRECTORYENTRYCOUNT ; i++ )
    {
        if( kMemCmp( pstRootEntry[ i ].vcDirName, pcFileName, iLength ) == 0 )
        {
...
static int kFindDirectoryEntry2( const char* pcFileName, DIRECTORYENTRY* pstEntry )
{
	//kPrintf("10%s",pcFileName);
...
static int kFindDirectoryEntry3( const char* pcFileName, DIRECTORYENTRY* pstEntry )
{
	//kPrintf("10%s",pcFileName);
...
int kMoveDirWrapper ( const char* pcDirName )
{
	return kMoveDir( pcDirName );
}

/**
 *  디렉터리에서 파일 이름이 일치하는 엔트리를 찾아서 인덱스를 반환
 */
static int kMoveDir ( const char* pcDirName )
{
    DIRECTORYENTRY* pstNowEntry;
    int i;
    int iLength;
    ...
    pstNowEntry = ( DIRECTORYENTRY* ) gs_vbTempBuffer;

    for( i = 0 ; i < FILESYSTEM_MAXDIRECTORYENTRYCOUNT ; i++ )
    {
	iLength = kStrLen( pstNowEntry[ i ].vcDirName );
	int tempLen = kStrLen( pcDirName );
	if( iLength == 0 ) 
		continue; 
	if( iLength < tempLen ) 
		iLength = tempLen;
	/*kPrintf(" Len : %d\n", iLength);
	kPrintf(" String : %s\n", pstNowEntry[ i ].vcDirName);
	kPrintf(" tempLen : %d\n", tempLen);
	kPrintf(" inputString : %s\n",  pcDirName );*/
        if( kMemCmp( pstNowEntry[ i ].vcDirName, pcDirName, iLength ) == 0 )
        {	
		if( pstNowEntry[ i ].dwStartClusterIndex == -1 ) nowDirOffset == 0;
		else nowDirOffset = pstNowEntry[ i ].dwStartClusterIndex;
            return i;
        }
    }
...
int kFindAbsolWrapper( char* pcDirName , const int DirOffset) 
{
	return kFindAbsol( pcDirName , DirOffset);
}

static int kFindAbsol( char* pcDirName , const int DirOffset) 
{
    DIRECTORYENTRY* pstNowEntry;
    int i;
    int iLength;
	...
	if( iLength < tempLen ) 
        ...
        if( kMemCmp( pstNowEntry[ i ].vcDirName, pcDirName, iLength ) == 0 )
        {	
            return pstNowEntry[ i ].dwStartClusterIndex;
        }
    }
...
BOOL kCreateDirWrapper(  char* tmp)
{
	//kPrintf("wrapper : %s\n",tmp);cre
	kCreateDir(tmp);
}

static BOOL kCreateDir(  char* pcDirName )
{
    	DWORD dwCluster;
	const char* myself = ".";
	const char* parent = "..";
	...
	BYTE zeroBuffer[ FILESYSTEM_SECTORSPERCLUSTER * 512 ] = {0,};
    ...
    {
		kUnlock( &( gs_stFileSystemManager.stMutex ) ); 
		kPrintf( "cluster error~!!!!\n" );

        return FALSE;
    }
	kWriteCluster(dwCluster, zeroBuffer);
	
	kMemCpy( pstEntry.vcDirName, pcDirName, kStrLen( pcDirName ) + 1 );
	...
	*piDirectoryEntryIndex = kFindFreeDirectoryEntry();
   	...
   	if( *piDirectoryEntryIndex == -1 )
   	{
		 ...
		 kUnlock( &( gs_stFileSystemManager.stMutex ) );
		 kPrintf( "directoryentry full~!!!!\n" );

       	 return FALSE;
   	}
	
	
	if( kSetDirectoryEntryData2(nowDirOffset,*piDirectoryEntryIndex, &pstEntry ) == FALSE )
  	{
     	   // 실패할 경우 할당 받은 클러스터를 반환해야 함
    	   	 kSetClusterLinkData( dwCluster, FILESYSTEM_FREECLUSTER );
		kPrintf( "setdirectoryentry error1~!!!!\n" );
	...
	for(i = 0 ; i < 2; i++)
	{
		 ...
		 *piDirectoryEntryIndex = kFindFreeDirectoryEntry2(dwCluster);
   		 if( *piDirectoryEntryIndex == -1 )
   		 {
       		 	// 실패할 경우 할당 받은 클러스터를 반환해야 함
      		 	kSetClusterLinkData( dwCluster, FILESYSTEM_FREECLUSTER );
			kUnlock( &( gs_stFileSystemManager.stMutex ) );
   			kPrintf( "directoryentry full~!!!!%d\n",i );
		...
		if(i == 0 ) //myself
		{
	    		kMemCpy( pstEntry.vcDirName, myself, kStrLen(myself) + 1 );
		...
		if(i == 1) //parent
		{
			kMemCpy(pstEntry.vcDirName, parent, kStrLen(parent) + 1 );
			kMemCpy( pstEntry.vcFileName, nowDirBuffer , kStrLen( nowDirBuffer ) + 1 );
			if(nowDirOffset == 0) pstEntry.dwStartClusterIndex = -1;
			else pstEntry.dwStartClusterIndex = nowDirOffset;
			pstEntry.dwFileSize = 0;
	    		pstEntry.isDir = '1';
		}
	    ...
	    // 디렉터리 엔트리를 등록
	    if( kSetDirectoryEntryData2(dwCluster, i, &pstEntry ) == FALSE )
  	    {
     	   // 실패할 경우 할당 받은 클러스터를 반환해야 함
    	    kSetClusterLinkData( dwCluster, FILESYSTEM_FREECLUSTER );
//			kUnlock( &( gs_stFileSystemManager.stMutex ) );
   
			kPrintf( "i setdirectoryentry error%d~!!!!\n",i );
...
BOOL kRemoveDirWrapper( char* dirName)
{
	kRemoveDir(dirName);
}

static BOOL kRemoveDir( char* dirName)
{
	DWORD dwCluster;
	...
	//kPrintf("1%s",dirName);
    // 파일 이름 검사
    iDirNameLength = kStrLen( dirName );
    if( ( iDirNameLength > ( sizeof( stEntry.vcDirName ) - 1 ) ) || 
        ( iDirNameLength == 0 ) )
    {
        return FALSE;
    }
	...
	iDirectoryEntryOffset = kFindDirectoryEntry2( dirName, &stEntry ); //
...
//	kPrintf("4%s",dirName);
    ...
    // 디렉터리를 읽음
    if( kReadCluster( stEntry.dwStartClusterIndex, gs_vbTempBuffer ) == FALSE )
    {
        return FALSE;
    }  
//	kPrintf("5%s",dirName);
	...
	kWriteCluster(stEntry.dwStartClusterIndex, zeroBuffer);
...
FILE* kOpenFile( const char* pcFileName, const char* pcMode )
{
    ...
    //==========================================================================
    iDirectoryEntryOffset = kFindDirectoryEntry2( pcFileName, &stEntry );
    if( iDirectoryEntryOffset == -1 )
    {
...
int kRemoveFile( const char* pcFileName )
{
    ...
    // 파일이 존재하는가 확인
    iDirectoryEntryOffset = kFindDirectoryEntry3( pcFileName, &stEntry );
    if( iDirectoryEntryOffset == -1 ) 
    { 
        // 동기화
        kUnlock( &( gs_stFileSystemManager.stMutex ) );
...
DIR* kOpenDirectory( const char* pcDirectoryName )
{
    ...
    // 루트 디렉터리를 읽음
    if( kReadCluster( nowDirOffset, ( BYTE* ) pstDirectoryBuffer ) == FALSE )
    {
    ...
    pstDirectory->stDirectoryHandle.iCurrentOffset = nowDirOffset;
