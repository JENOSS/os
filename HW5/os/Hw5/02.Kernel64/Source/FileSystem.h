...
{
    // 파일 이름 // 디렉터리 입장에서는 여분의 이름
    char vcFileName[ FILESYSTEM_MAXFILENAMELENGTH ];
	
    // 자신을 표현하는 디렉터리 엔트리 이름 
    char vcDirName[ FILESYSTEM_MAXFILENAMELENGTH ];
...
/*   전역변수 영역   */
//현재 자신이 있는  디렉터리 이름 
static BYTE nowDirBuffer[ 300 ]; 

//현재 자신이 있는 디렉터리의 클러스터 Offset
...
static BOOL kSetDirectoryEntryData2(DWORD dwClusterIndex, int iIndex, DIRECTORYENTRY* pstEntry );
...
int kFMoveDirWrapper ( const char* pcDirName );
